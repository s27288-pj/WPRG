...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        setCookieValue('quantity', validate_input($_POST['quantity']));
        setCookieValue('address', validate_input($_POST['address']));
        setCookieValue('credit_card', validate_input($_POST['credit_card']));
        setCookieValue('arrival_date', validate_input($_POST['arrival_date']));
        setCookieValue('departure_date', validate_input($_POST['departure_date']));
        setCookieValue('arrival_time', validate_input($_POST['arrival_time']));
        setCookieValue('child_bed', isset($_POST['child_bed']) ? '1' : '');

        if (isset($_POST['amenities'])) {
            setCookieValue('amenities', implode(',', $_POST['amenities']));
        } else {
            setCookieValue('amenities', '');
        }
        ...
        for ($i = 1; $i <= $quantity; $i++) {
            ...
            setCookieValue('first_name_' . $i, validate_input($_POST['first_name_' . $i]));
            setCookieValue('last_name_' . $i, validate_input($_POST['last_name_' . $i]));
            setCookieValue('email_' . $i, validate_input($_POST['email_' . $i]));
            setCookieValue('phone_' . $i, validate_input($_POST['phone_' . $i]));
        ...
        if (empty($errors)) {
            ...
            echo "<h2>Menu:</h2>";
            echo "<p>Chcesz powrócić do formularza rezerwacji?.</p>";
            echo "<a href='reservation.php'><button>Wróć</button></a>";
            echo "<p>Chcesz się wylogować?.</p>";
            ...
            echo "<h2>Menu:</h2>";
            echo "<p>Chcesz powrócić do formularza by poprawić błędy rezerwacji?.</p>";
            echo "<a href='reservation.php'><button>Wróć</button></a>";
            echo "<p>Chcesz się wylogować?.</p>";
            echo "<a href='logout.php'><button>Wyloguj</button></a>";
        }
